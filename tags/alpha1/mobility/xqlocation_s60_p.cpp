...
/*
bool XQLocationPrivate::GetSatelliteData(int id, XQSatInfo &satinfo)
{
    return false;
}
*/
void XQLocationPrivate::DeliverPositionerResults(TPositionSatelliteInfo aPositionInfo)
{
    ...
    if (heading != iPreviousHeading) {
        emit ipParent->headingChanged(heading);
    }
    iPreviousHeading = heading;
    ...
    if (satelliteInfoAvailable) {
        ...
        for (int i=0; i < aPositionInfo.NumSatellitesInView(); i++)
		{
			if (aPositionInfo.GetSatelliteData(i,satdata) == KErrNone)
				emit ipParent->updateSatInfo(
						satdata.SatelliteId(),
						satdata.SignalStrength(),
						satdata.Azimuth(),
						satdata.Elevation(),
						satdata.IsUsed()
						);
    ...
    if (iPreviousPosition.Latitude() != pos.Latitude() ||
        ...
        if (!isnan(pos.Latitude()) || !isnan(pos.Longitude()) || !isnan(pos.Altitude())) {
            emit ipParent->locationChanged(pos.Latitude(),pos.Longitude(),pos.Altitude(),speed,heading);
        }
